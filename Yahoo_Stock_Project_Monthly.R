...
#read from csv file
#
df_stock <-read.csv("/home/wchang/Downloads/quantquote_daily_sp500_83986/daily/FLWS.csv", header = TRUE)
df_stock <- df_stock[complete.cases(df_stock),]
...
spliter <- function(df_stock, i) {
  ...
  df_temp <- data.frame()
  df_temp <- df_stock %>%
    filter(substr(df_stock$Date, 6,7) == i)
  return(df_temp)
}  
...
for (i in 1:nrow(df_buy_stock)) {
  df_buy_sell <- rbind(df_buy_sell, rbind(c("Buy_Date"=as.character(df_buy_stock$Date[[i]]), 
                                            ...
                                            "Profit" = (df_sell_stock$Open[[i]]-df_buy_stock$Open[[i]])/df_buy_stock$Open[[i]] + 1
                                            )))
}
